...
interface Ball <in T: TUpper>
{
  fun set(a: T)
}

fun sample(foo: Foo<*>, ball: Ball<*>){//星形投影只能读取，不能写入，因为in时，Nothing，out时，Any或者Tupper
  var tUpper:TUpper = foo.get()
//  ball.set(tUpper) //ball.set(Nothing)
}

//T必须是CharSequence和Comparable<T>的子类
